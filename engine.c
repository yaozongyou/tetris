...
const shapes_t SHAPES = {
    ...
    /*
     *   *                   *                                         *                   *
     *  *#*  anti-clockwise  #*  anti-clockwise  *#*  anti-clockwise  *#  anti-clockwise  *#*
     *                       *                    *                    *
     */
...
static int droplines(board_t board) {
    ...
    for (i = 0; i < NUMCOLS; i++) {
        newboard[i][NUMROWS - 1] = newboard[i][NUMROWS - 2] = WALL;
    }
    for (i = 0; i < NUMROWS; i++) {
        newboard[0][i] = newboard[NUMCOLS - 1][i] = newboard[NUMCOLS - 2][i] = WALL;
    }
    /* ... */
    ny = NUMROWS - 3;
    droppedlines = 0;
...
void engine_init(engine_t* engine, void (*score_function)(engine_t *)) {
    ...
    engine->score = 0;
    engine->status.moves = engine->status.rotations = engine->status.dropcount = 
        engine->status.efficiency = engine->status.droppedlines = 0;
    /* initialize shapes */
    memcpy(engine->shapes, SHAPES, sizeof(shapes_t));
    /* initialize board */
    memset(engine->board, 0, sizeof(board_t));
    for (i = 0; i < NUMCOLS; i++) {
        engine->board[i][NUMROWS - 1] = engine->board[i][NUMROWS - 2] = WALL;
    }
    for (i = 0; i < NUMROWS; i++) {
        engine->board[0][i] = engine->board[NUMCOLS - 1][i] = engine->board[NUMCOLS - 2][i] = WALL;
    }
}
...
void engine_move(engine_t* engine, action_t action) {
    ...
    case ACTION_LEFT:
        if (shape_left(engine->board, &engine->shapes[engine->curshape], 
    ...
    case ACTION_ROTATE:
        if (shape_rotate(engine->board, &engine->shapes[engine->curshape],
                    engine->curx, engine->cury)) {
    ...
    case ACTION_RIGHT:
        if (shape_right(engine->board,&engine->shapes[engine->curshape],
                    &engine->curx,engine->cury)) {
            engine->status.moves++;
        }
        break;
    ...
    case ACTION_DROP:
        engine->status.dropcount += shape_drop(engine->board, &engine->shapes[engine->curshape], 
                engine->curx, &engine->cury);
    }
}
...
int engine_evaluate(engine_t* engine) {
    if (shape_bottom(engine->board,&engine->shapes[engine->curshape],engine->curx,engine->cury)) {
        ...
        engine->status.efficiency += engine->status.dropcount + engine->status.rotations + 
            (engine->curx - engine->status.moves);
        ...
        /* return games status */
        return allowed(
                engine->board, &engine->shapes[engine->curshape], engine->curx, engine->cury) ? 
            0 : -1;
    }
